...
class GUI {
    ...
    private initialize() {
        ...
        sceneFolder.add(this.properties, "sceneId", {
            ...
            "Explore": GAME.Scenes.EXPLORE,
            "Game": GAME.Scenes.GAME
        }).name("Scene")
            .onChange((x) => this.properties.sceneId = +x);
        sceneFolder.open();
        ...
        var characterCtr = gameFolder.add(this.properties.gameParameters, "character",
            ["fox", "wolf","deer", "elk", "mountainlion", "chowchow", "goldenRetreiver", "moose"]).name("Character").onChange((x) => this.properties.gameParameters.character = x);
        gameFolder.open();
